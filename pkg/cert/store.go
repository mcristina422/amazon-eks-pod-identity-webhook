...
func (s *secretCertStore) Update(cert, key []byte) (*tls.Certificate, error) {
	...
	if err != nil {
		...
		_, err = s.clientset.CoreV1().Secrets(s.namespace).Create(context.Background(), secret, metav1.CreateOptions{})
		if err != nil {
	...
	_, err = s.clientset.CoreV1().Secrets(s.namespace).Update(context.Background(), secret, metav1.UpdateOptions{})
	if err != nil {
